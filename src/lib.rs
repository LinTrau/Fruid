...
impl FlipFluid {
    fn new(
        ...
        let s = vec![0.0; number_of_cells_setting];
    ...
    fn showParticles(&mut self) {
        ...
        for i in 0..self.numParticles as usize {
            ...
            if cell_location_x < self.fNumX as usize && cell_location_y < self.fNumY as usize {
                self.cellType[cell_location_x * n + cell_location_y] = CellType::FLUID_CELL;
            }
        }
    }
...
impl Scene {
    pub fn setupScene(num_particles: i32, num_rows: i32, num_cols: i32) -> Scene {
        let xGravity = 0.0;
        let yGravity = 0.0;
        let flipRatio = 0.85;
        let frameNr = 0;
        ...
        let mut fluid = FlipFluid::new(density, tankWidth, tankHeight, h, r, num_particles, num_rows, num_cols);
...
#[unsafe(no_mangle)]
pub extern "C" fn setup_scene_wrapper(num_particles: i32, num_rows: i32, num_cols: i32) -> *mut Scene {
    let scene = Box::new(Scene::setupScene(num_particles, num_rows, num_cols));
    Box::into_raw(scene)
}
